  ...
  return (
    <div className="bg-white dark:bg-[#121215] rounded-2xl border border-slate-200 dark:border-zinc-800/80 shadow-sm p-5 sm:p-6 mb-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-5 border-b border-slate-100 dark:border-zinc-800">
        <div>
          {/* Previous Page Return Button */}
          {onGoBack && (
            <div className="mb-2.5">
              <button
                id="exec-summary-back-btn"
                type="button"
                onClick={onGoBack}
                className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-indigo-50 dark:bg-indigo-500/15 hover:bg-indigo-600 hover:text-white dark:hover:bg-indigo-600 text-indigo-700 dark:text-indigo-300 font-extrabold text-xs transition-all cursor-pointer border border-indigo-200/80 dark:border-indigo-500/30 shadow-2xs group active:scale-95"
                title={`ย้อนกลับไปหน้า: ${previousViewLabel || 'หน้าก่อนหน้า'}`}
              >
                <ArrowLeft className="w-3.5 h-3.5 group-hover:-translate-x-0.5 transition-transform" />
                <span>← ย้อนกลับไป{previousViewLabel ? `หน้า ${previousViewLabel}` : 'หน้าก่อนหน้า'}</span>
              </button>
            </div>
          )}
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-1.5">
            ...
            <span className="px-2.5 py-0.5 text-xs font-bold rounded-lg bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20 font-mono">
              {stock.dataStatus || 'REAL_EOD'} • {stock.priceDate || '27/08/2026'}
            </span>
          ...
          <div className="flex items-center gap-4 bg-slate-50 dark:bg-[#18181B] px-4 py-3 rounded-xl border border-slate-200/80 dark:border-zinc-800">
            <div>
              <div className="text-xs text-slate-400 dark:text-zinc-500 font-medium">
                {stock.isMarketOpen ? 'ราคาซื้อขายล่าสุด (Live Last)' : 'ราคาล่าสุด / ปิดจริง (Current Last)'}
              </div>
              <div className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white">
                {(stock.currentLast || stock.currentPrice).toFixed(2)} <span className="text-sm font-semibold text-slate-500 dark:text-zinc-400">{stock.currency}</span>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 my-5">
        ...
        <div className="p-4 rounded-xl border bg-slate-50/70 dark:bg-[#18181B]/70 border-slate-200/80 dark:border-zinc-800 relative overflow-hidden">
          ...
          <div className="space-y-1.5 text-xs text-slate-600 dark:text-zinc-300">
            ...
            {stock.assetCategory !== 'FOREX' && !stock.forexMacro && (
              <div className="pt-2 mt-2 border-t border-slate-200/80 dark:border-zinc-800">
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="w-full text-[11px] font-bold py-1.5 px-2 rounded-lg bg-teal-50 hover:bg-teal-100 dark:bg-teal-950/40 dark:hover:bg-teal-900/50 text-teal-700 dark:text-teal-300 flex items-center justify-center space-x-1.5 transition-colors border border-teal-200/60 dark:border-teal-800/40 cursor-pointer shadow-2xs"
                >
                  <TrendingUp className="w-3.5 h-3.5 text-teal-600 dark:text-teal-400" />
                  <span>ดูกราฟงบ 5 ปี (กำไร / หนี้ / เพิ่มทุน) ↓</span>
                </button>
              </div>
            )}
          </div>
      ...
      <div className="p-4 rounded-xl bg-slate-50 dark:bg-[#18181B] border border-slate-200/70 dark:border-zinc-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="shrink-0 flex items-center space-x-2 w-full md:w-auto">
          {stock.assetCategory !== 'FOREX' && !stock.forexMacro && (
            <button
              id="btn-jump-to-governance"
              onClick={() => {
                document.getElementById('section-executive-governance')?.scrollIntoView({ behavior: 'smooth' });
              }}
              className="px-3.5 py-2.5 rounded-xl bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700 text-slate-800 dark:text-zinc-200 font-bold text-xs sm:text-sm transition-all flex items-center justify-center space-x-1.5 cursor-pointer shadow-xs"
              title="ตรวจสอบธรรมาภิบาลและข่าวสาร ก.ล.ต."
            >
              <ShieldCheck className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
              <span>ธรรมาภิบาลผู้บริหาร</span>
            </button>
          )}
          <button
