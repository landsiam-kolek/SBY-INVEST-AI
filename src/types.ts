...
/**
 * Phase 2 Master Data Quality Grades (Strict Zero-Fake-Data Standard)
 * GRADE_A: Real + Recent (Live/Delayed) + Complete
 * GRADE_B: Real + Verified EOD + Sufficient Historical Data (>= 50 candles)
 * GRADE_C: Real but Incomplete (< 50 candles or missing optional metrics)
 * GRADE_D: Unavailable / Provider Not Configured
 * GRADE_F: Synthetic / Unverified / Blocked (Forbidden in Analysis)
 */
export type DataQualityGrade = 'GRADE_A' | 'GRADE_B' | 'GRADE_C' | 'GRADE_D' | 'GRADE_F';

export type ProviderStatus = 
  | 'ACTIVE_STREAM'
  | 'VERIFIED_EOD_AVAILABLE'
  | 'USER_IMPORT_AVAILABLE'
  | 'PROVIDER_NOT_CONFIGURED'
  | 'PROVIDER_ERROR';

/**
 * Master Data Statuses (Strict Zero-Fake-Data Standard)
 */
...
export interface ValidatedCandleSeries {
  symbol: string;
  market: MarketType;
  ...
  dataSource: string;
  isSufficientForTechnical: boolean; // true if totalCandles >= 50
  validationErrors?: string[];
}

export interface AssetMarketMetadata {
  symbol: string;
  name: string;
  market: MarketType;
  ...
  currency: 'THB' | 'USD' | string;
  tickSize: number;
  tradeCalendar: 'SET_BANGKOK' | 'US_NEWYORK' | 'FOREX_24H';
...
export interface NormalizedMarketData {
  ...
  market: MarketType;
  currency: 'THB' | 'USD' | string;
  currentLast?: number;       // Only populated when real live stream is connected
  officialEODClose: number;   // Official closing price of trade date
  previousClose: number;     // Previous day official close
  change: number;
  changePercent: number;
  volume: number;
  tradeDate: string;         // YYYY-MM-DD
  timestamp: string;         // ISO 8601
  dataSource: string;        // e.g. 'SIAMCHART_EOD', 'SET_SNAPSHOT', 'USER_CSV'
  dataQuality: DataQualityGrade;
...
export interface StockData {
  ...
  currentPrice: number;
  // Explicit Price Semantics for AI & Analysis Engine (V1.0 Precision Standards)
  currentLast?: number;       // ราคาซื้อขายล่าสุดจริง (Current Live Last Price - Only when live feed active)
  officialEODClose?: number;  // ราคาปิดจริงทางการของวันทำการ (Official EOD Close)
  close?: number;             // ราคาปิดจริงของวันทำการนั้น (Official Trade Date Close)
  previousClose?: number;     // ราคาปิดจริงของวันทำการก่อนหน้า (Official Previous Day Close)
  priceDate?: string;         // วันที่ของราคา (e.g. "27/08/2026")
  dataSource?: 'SIAMCHART' | 'SETTRADE' | 'MANUAL_VALIDATED' | 'OFFICIAL_SET' | string;
  dataStatus?: MarketDataStatus; // REAL_EOD | REAL_LIVE | USER_IMPORTED | DATA_UNAVAILABLE | INSUFFICIENT_DATA | PROVIDER_NOT_CONFIGURED
  dataQuality?: DataQualityGrade; // GRADE_A | GRADE_B | GRADE_C | GRADE_D | GRADE_F
  providerStatus?: ProviderStatus; // ACTIVE_STREAM | VERIFIED_EOD_AVAILABLE | USER_IMPORT_AVAILABLE | PROVIDER_NOT_CONFIGURED
  fundamentalStatus?: FundamentalDataStatus; // VERIFIED_AVAILABLE | FUNDAMENTAL_DATA_UNAVAILABLE | INSUFFICIENT_DATA
  orderBookStatus?: OrderBookStatus; // ORDER_BOOK_AVAILABLE | ORDER_BOOK_DATA_UNAVAILABLE
