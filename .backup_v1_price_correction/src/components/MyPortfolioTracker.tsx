...
function generateCustomStockData(
  ...
  const now = new Date();
  const candles = [];
  let p = currentPrice * 0.94;
  for (let i = 30; i >= 0; i--) {
    const d = new Date(now.getTime() - i * 24 * 60 * 60 * 1000);
    const change = (Math.random() - 0.48) * (currentPrice * 0.02);
    p = Math.max(currentPrice * 0.7, p + change);
    if (i === 0) p = currentPrice;
    candles.push({
      date: d.toISOString().split('T')[0],
      open: Number((p * 0.995).toFixed(2)),
      high: Number((p * 1.015).toFixed(2)),
      low: Number((p * 0.985).toFixed(2)),
      close: Number(p.toFixed(2)),
      volume: Math.floor(1500000 + Math.random() * 4000000),
      ema20: Number((p * 0.99).toFixed(2)),
      ema50: Number((p * 0.97).toFixed(2)),
      ema200: Number((p * 0.94).toFixed(2)),
      rsi: 54,
    });
  }
