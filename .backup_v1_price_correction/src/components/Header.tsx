...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 dark:bg-[#000000]/95 backdrop-blur-md border-b border-slate-200 dark:border-zinc-800/80 transition-colors">
      ...
      <div className={`text-zinc-300 text-[11px] py-1 px-4 sm:px-6 lg:px-8 flex items-center justify-between border-b transition-colors ${
        ...
        <div className="flex items-center space-x-2 truncate">
          {tradingMode === 'DAY_TRADE' ? (
            <span className="inline-flex items-center space-x-1.5 font-black text-amber-300 animate-pulse">
              ...
              <span>⚡ โหมดปัจจุบัน: DAY TRADE & SWING (เก็งกำไรความเสี่ยงสูง 1 วัน - 1 สัปดาห์)</span>
            </span>
          ) : (
            <span className="inline-flex items-center space-x-1 font-bold text-amber-400">
              <span>💎 โหมดปัจจุบัน: VI & จัดพอร์ตระยะกลาง-ยาว</span>
            </span>
          )}
        ...
        <div className="flex items-center space-x-2.5 sm:space-x-4">
          ...
          <button
            ...
            className={`px-2.5 py-0.5 rounded-full text-[10px] font-black transition-all flex items-center space-x-1 cursor-pointer ${
              tradingMode === 'DAY_TRADE'
          ...
          >
            {tradingMode === 'DAY_TRADE' ? (
              <>
                <span>🔄 สลับกลับโหมด VI & จัดพอร์ต</span>
              </>
            ) : (
              <>
                <Zap className="w-3 h-3 text-amber-400" />
                <span>⚡ สลับเป็นโหมด Day Trade (1วัน-1wk)</span>
              </>
            )}
          </button>
          ...
          {authUser && (
            <div className="flex items-center space-x-1.5">
              <span className="text-[10px] font-black px-1.5 py-0.2 rounded bg-indigo-500/20 text-indigo-300">
                {authUser.username}
              </span>
            </div>
          )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-2 sm:gap-4">
        ...
        <div className="flex items-center space-x-3 sm:space-x-5 shrink-0">
          ...
          >
            <SataRobotLogo size={38} glow={true} className="group-hover:scale-105 transition-transform" />
            <div>
              <div className="font-black text-base sm:text-lg tracking-tight text-slate-900 dark:text-white leading-none flex items-center space-x-1.5">
                <span>SBY Invest</span>
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 font-bold border border-indigo-200 dark:border-indigo-500/30">
                  AI
                </span>
              </div>
              <div className="text-[10px] font-semibold text-slate-400 dark:text-zinc-500 leading-tight hidden sm:block">
          ...
          <div className="flex items-center bg-slate-100 dark:bg-[#18181B] p-1 rounded-2xl border border-slate-200/80 dark:border-zinc-800 text-xs font-bold">
            <button
              onClick={() => onChangeView('analysis')}
              id="nav-tab-analysis"
              className={`px-2.5 sm:px-3 py-1.5 rounded-xl transition-all flex items-center space-x-1.5 ${
            ...
            >
              ...
              <span className="hidden sm:inline">วิเคราะห์รายตัว</span>
            ...
            >
              ...
              <span className="hidden sm:inline">พอร์ตติดตามเอง</span>
              <span className="sm:hidden">พอร์ตของฉัน</span>
            </button>
            ...
            >
              ...
              <span className="hidden md:inline">AI จัดพอร์ต (5-8 ตัว)</span>
              <span className="md:hidden">AI จัดพอร์ต</span>
            </button>

            {/* NEW 4TH TAB: DEDICATED DAY TRADE TERMINAL */}
            <button
              onClick={() => {
                onToggleTradingMode('DAY_TRADE');
                onChangeView('day-trade');
              ...
              className={`px-2.5 sm:px-3 py-1.5 rounded-xl transition-all flex items-center space-x-1.5 font-black ${
                currentView === 'day-trade'
                  ? 'bg-amber-500 text-slate-950 shadow-md shadow-amber-500/20'
                  : 'text-amber-600 dark:text-amber-400 hover:bg-amber-500/10'
              }`}
            >
              <Zap className={`w-3.5 h-3.5 ${currentView === 'day-trade' ? 'text-slate-950' : 'text-amber-500'}`} />
              <span className="hidden lg:inline">⚡ โหมด Day Trade (1วัน-1wk)</span>
              <span className="lg:hidden">⚡ Day Trade</span>
            </button>
          </div>
        </div>

        {/* Global Search Bar with Autocomplete & Custom AI Stock Lookup */}
        <div ref={dropdownRef} className="relative flex-1 max-w-xs md:max-w-md hidden sm:block">
          <div className="relative">
            ...
            <input
              ...
              placeholder="ค้นหาหุ้น/สินทรัพย์ เช่น CPALL, NVDA, XAU/USD..."
              className="w-full pl-9 pr-20 py-2 text-xs rounded-xl bg-slate-100 dark:bg-[#121215] border border-slate-200 dark:border-zinc-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-slate-900 dark:text-zinc-100 placeholder-slate-400 dark:placeholder-zinc-500 transition-all font-medium"
            />
            {searchQuery && (
              <button
                onClick={handleCustomSearchSubmit}
                disabled={isSearchingCustom}
                className="absolute right-1.5 top-1/2 -translate-y-1/2 px-2 py-1 text-[11px] bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-500 transition-all flex items-center space-x-1"
              >
        ...
        <div className="flex items-center space-x-1.5 sm:space-x-2 shrink-0">
          ...
          {onOpenAddTickerModal && (
            <button
              ...
              className="flex items-center space-x-1.5 px-2.5 sm:px-3 py-2 text-xs font-black rounded-xl border border-emerald-500/40 bg-emerald-500/15 hover:bg-emerald-600 hover:text-white text-emerald-700 dark:text-emerald-300 transition-all shadow-xs cursor-pointer"
          ...
          {onOpenPriceSyncModal && (
            <button
              ...
              onClick={onOpenPriceSyncModal}
              className="flex items-center space-x-1.5 px-2.5 sm:px-3 py-2 text-xs font-black rounded-xl border border-indigo-400/80 dark:border-indigo-500/50 bg-indigo-500/15 dark:bg-indigo-500/10 hover:bg-indigo-600 hover:text-white text-indigo-700 dark:text-indigo-300 transition-all shadow-xs cursor-pointer group"
              title="เปิดศูนย์รายงานราคาปิดประจำวัน (Daily Stock Price Report), Export Excel/PDF, และนำเข้า Siamchart"
            >
              <FileSpreadsheet className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400 group-hover:text-white" />
              <span className="hidden sm:inline">รายงานราคาปิด (Last)</span>
            </button>
          )}

          {/* Investment Goal / Objective Button */}
          <button
            id="open-objective-modal-btn"
            onClick={onOpenObjectiveModal}
            className="flex items-center space-x-1.5 px-2.5 sm:px-3 py-2 text-xs font-bold rounded-xl border border-indigo-200 dark:border-indigo-500/30 bg-indigo-50/50 dark:bg-indigo-500/10 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 transition-all shadow-xs"
          ...
          <button
            ...
            className="flex items-center space-x-1.5 px-2.5 sm:px-3 py-2 text-xs font-semibold rounded-xl border border-slate-200 dark:border-zinc-800 bg-white dark:bg-[#121215] hover:bg-slate-50 dark:hover:bg-zinc-800 text-slate-700 dark:text-zinc-200 transition-all shadow-xs"
          ...
          <button
            ...
            className="relative flex items-center space-x-1.5 px-2.5 sm:px-3 py-2 text-xs font-semibold rounded-xl border border-slate-200 dark:border-zinc-800 bg-white dark:bg-[#121215] hover:bg-slate-50 dark:hover:bg-zinc-800 text-slate-700 dark:text-zinc-200 transition-all shadow-xs"
          ...
          <button
            ...
            className="flex items-center space-x-1.5 px-3 py-2 text-xs font-bold rounded-xl bg-gradient-to-r from-indigo-600 via-blue-600 to-cyan-600 hover:from-indigo-500 hover:to-cyan-500 text-white shadow-md shadow-indigo-500/20 transition-all"
          ...
          <button
            ...
            className="p-2 rounded-xl border border-slate-200 dark:border-zinc-800 bg-white dark:bg-[#121215] hover:bg-slate-50 dark:hover:bg-zinc-800 text-slate-600 dark:text-zinc-300 transition-all"
          ...
          {authUser && (
            <div ref={userMenuRef} className="relative">
              <button
                ...
                className="p-2 rounded-xl border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-800/80 hover:bg-slate-100 dark:hover:bg-zinc-700 text-slate-700 dark:text-zinc-200 transition-all"
              ...
              {isUserMenuOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-[#121215] border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-xl p-3 z-50 text-xs">
